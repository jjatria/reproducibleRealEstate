...
setAlpha <- function (
  ...
  if (length(alpha) != 1 & length(alpha) != length(col)){
    stop(
      'Please provide a single alpha value ',
      'OR a value for each color in the vector'
    )
  }
...
shinyServer(function (input, output) {
  ...
  updateData <- reactive({
    ...
    sales <- subset(kcSales,
      SalePrice <= as.numeric(input$priceLimits[2]) &
      SalePrice >= as.numeric(input$priceLimits[1])
    )

    sales <- subset(sales,
      SqFtLot <= as.numeric(input$lotSizeLimits[2]) &
      SqFtLot >= as.numeric(input$lotSizeLimits[1])
    )

    sales <- subset(sales,
      SqFtTotLiving <= as.numeric(input$homeSizeLimits[2]) &
      SqFtTotLiving >= as.numeric(input$homeSizeLimits[1])
    )

    sales <- subset(sales,
      Bedrooms <= as.numeric(input$bedLimits[2]) &
  ...
  buildSWM <- eventReactive(input$rerun, {
    if (input$spatEcon){
      dataSP <- SpatialPointsDataFrame(
        coords = cbind(updateData()$X, updateData()$Y),
        data = updateData()
      )
  ...
  createCoefTable <- eventReactive(input$rerun, {
    ...
    modRes <- try(estimateModel(), silent = TRUE)

    filterError <- function() {
      xtable(data.frame(
        message = paste0(
          'You have filtered out too many sales, ',
          'include more sales and try again'
        )
      ))
    }
    ...
    if (class(modRes) == 'try-error') {
      ...
      if (class(modRes) == 'sarlm'){
        valTable <- data.frame(
          Coef  = modRes$coefficients,
          StErr = modRes$rest.se
        )

        errorCheck <- try(
          rownames(valTable) <- c('Constant', rNames), silent=TRUE
        )
      } else {
  ...
  output$valPlot2 <- renderPlot({
    coefs <- estimateModel()
      ...
      barplot(
        100 * (exp(coefs$coefficients[sel]) - 1),
        names.arg = vNames,
        las       = 3,
        col       = vCol,
        border    = vCol,
  ...
  output$diagTable <- renderTable({
    ...
    if (class(mod) == 'sarlm') {
      ...
      diags  <- data.frame(
        Diagnostic = c(
          'R-squared', 'Std Error', 'Mean Absolute Error', 'Lambda', 'AIC'
        ),
      ...
      diags  <- data.frame(
        Diagnostic = c(
          'R-squared', 'Std Error', 'Mean Absolute Error', 'F-Stat', 'AIC'
        ),
  ...
  output$resPlot <- renderPlot({
    ...
    legend(
      'bottomright',
      c('Over Predict', 'Under Predict', 'Dark=higher error'),
      col = c(2,4,0), pch = 16
    )
  })
})

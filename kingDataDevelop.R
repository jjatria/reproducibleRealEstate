...
kngBuildAssdVal <- function(
    avYears,            # Years to use
  ...
  if (verbose) message('Reading in raw data')
  assValConn <- dbConnect(RSQLite::SQLite(), assdValDB)
  avData <- dbGetQuery(
    assValConn,
    paste0(
      'SELECT Major, Minor, TaxYr, ',
        'ApprLandVal AS LandVal, ',
        'ApprImpsVal AS ImpsVal ',
      'FROM ValueHistory ',
      'WHERE TaxYr >= ', avYears[1], ' ',
        'AND TaxYr <= ', avYears[length(avYears)], ' ',
        'AND Reason = "REVALUE"'
    )
  )
  ...
  for(avY in avYears){
    ...
    if (overWrite & tExists) {
      ...
      if (verbose) message('    Removing existing table: ', avName)
    }
    dbWriteTable(assValConn, avName, kngBuildPinx(avData[avData$TaxYr == avY, ])
                                                  , row.names=FALSE)
    if (verbose) message('Writing out ', avName)
...
kngBuildPinx <- function(
    X,                     # Dataset to use, including Major, Minor
